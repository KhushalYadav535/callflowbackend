...
const parseAccounts_1 = require("../utils/parseAccounts");
...
router.post('/tenant/:companyId/accounts', async (req, res) => {
    try {
        ...
        const ds = await DataSourceConfig_1.DataSourceConfig.findOne({ companyId: companyObjectId });
        if (!ds || ds.mode !== 'push') {
        ...
        const expectedSig = crypto_1.default.createHmac('sha256', secret).update(bodyForHmac).digest('hex');
        const providedSig = sigHeader.replace(/^sha256=/, '');
        if (!crypto_1.default.timingSafeEqual(Buffer.from(expectedSig, 'hex'), Buffer.from(providedSig, 'hex'))) {
        ...
        for (const item of payload) {
            ...
            const existing = await AccountProfile_1.AccountProfile.findOne({ companyId: companyObjectId, externalAccountId });
            if (eventType === 'account_closed') {
                if (existing) {
                    await AccountProfile_1.AccountProfile.updateOne({ _id: existing._id }, { $set: { status: 'COMPLETED', dataFreshnessAt: eventTs } });
                    await DispatchQueue_1.DispatchQueue.deleteOne({ companyId: companyObjectId, accountId: existing._id });
                    closed++;
                }
                continue;
            }
            const phoneRaw = fields.phone || fields.customerPhone || existing?.phone || '';
            const phone = phoneRaw ? (0, phoneNormalize_1.normalisePhone)(String(phoneRaw)) : (existing?.phone || '');
            const doc = {
            ...
            if (fields.dpd !== undefined)
                doc.dpd = Number(fields.dpd);
            if (fields.outstandingAmount !== undefined)
                doc.outstandingAmount = Number(fields.outstandingAmount);
            if (fields.dueDate !== undefined)
                doc.dueDate = fields.dueDate ? new Date(fields.dueDate) : null;
            if (fields.maturityDate !== undefined)
                doc.maturityDate = fields.maturityDate ? new Date(fields.maturityDate) : null;
            if (fields.kycExpiryDate !== undefined)
                doc.kycExpiryDate = fields.kycExpiryDate ? new Date(fields.kycExpiryDate) : null;
            if (fields.productType !== undefined)
                doc.productType = fields.productType;
            if (fields.email !== undefined)
                doc.email = fields.email;
            if (fields.altPhone !== undefined)
                doc.altPhone = fields.altPhone;
            ...
            if (existing) {
                ...
                await AccountProfile_1.AccountProfile.create({
                    ...doc,
        ...
        await DataSourceConfig_1.DataSourceConfig.updateOne({ companyId: companyObjectId }, { $set: { lastSyncAt: new Date(), lastSyncStatus: 'success' } });
...
router.post('/tenant/:companyId/accounts/upload', auth_1.authMiddleware, upload.single('file'), async (req, res) => {
    try {
        ...
        const companyObjectId = new mongoose_1.default.Types.ObjectId(companyIdParam);
        if (!req.file) {
        ...
        let accounts;
        try {
            accounts = (0, parseAccounts_1.parseAccountsFromBuffer)(req.file.buffer);
        }
        catch (parseErr) {
        ...
        for (const acc of accounts) {
            const existing = await AccountProfile_1.AccountProfile.findOne({
                companyId: companyObjectId,
        ...
        await DataSourceConfig_1.DataSourceConfig.findOneAndUpdate({ companyId: companyObjectId }, {
            $set: {
                mode: 'file',
                lastSyncAt: now,
...
router.post('/sync/pull/:companyId', auth_1.authMiddleware, async (req, res) => {
    try {
        const companyIdParam = req.params.companyId;
        const companyIdFromAuth = req.companyId;
        if (!companyIdFromAuth || companyIdFromAuth !== companyIdParam) {
        ...
        const companyObjectId = new mongoose_1.default.Types.ObjectId(companyIdParam);
        const { runPullSync } = await Promise.resolve().then(() => __importStar(require('../services/pullSyncService')));
        ...
        const companyObjectId = req.params.companyId ? new mongoose_1.default.Types.ObjectId(req.params.companyId) : null;
        if (companyObjectId) {
            await DataSourceConfig_1.DataSourceConfig.updateOne({ companyId: companyObjectId }, { $set: { lastSyncStatus: 'failed' } }).catch(() => { });
            const { writeCallEvent } = await Promise.resolve().then(() => __importStar(require('../services/eventWriter')));
        ...
        const msg = err && typeof err === 'object' && 'response' in err ? err.response?.status === 401 ? 'CBS auth failed' : 'CBS API error' : 'Failed to trigger pull sync';
