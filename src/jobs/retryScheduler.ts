...
async function runRetryDispatch() {
  try {
    ...
    const byCampaign = new Map<string, typeof dueContacts>()
    for (const c of dueContacts) {
      const cid = (c.campaignId as any)?._id?.toString()
      if (cid) {
    ...
    for (const [campaignIdStr, contacts] of byCampaign) {
      ...
      for (let b = 0; b < batches.length; b++) {
        await Promise.all(
          batches[b].map((body) =>
            axios.post(n8nUrl, body).catch((err) => console.error('Retry scheduler: failed n8n for', body.contactId, err))
          )
        )
        await Contact.updateMany(
          { _id: { $in: batches[b].map((p) => p.contactId) } },
